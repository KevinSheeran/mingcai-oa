	...
	<div class="page__bd">
		...
		<c:forEach items="${itmelist}" var="item">
			<div class="weui-cells">
					...
					<div class="weui-cell">
						...
						<div class="weui-cell__ft">${item.eosZId}</div>
					</div>
			</div>
		</c:forEach>
		...
		<c:if test="${flow.status!=null}">
			...
			<div class="weui-btn-area">
				...
				<a href="javascript:;" class="weui-btn <c:if test="${oepro.status!=2}"> disabled</c:if> weui-btn_default" oeproid="${oepro.id}" flowid="${flow.id}"  id="backIOSDialog">驳回</a>
			</div>
		</c:if>
	</div>
</div>
<script type="application/javascript">
    ...
    $('#backIOSDialog').on('click', function(){
        ...
        var flowid=$(this).attr("flowid");
        var btnArray = ['否', '是'];
        mui.confirm('确认驳回预立项审核吗？', '系统提示', btnArray, function(e) {
            if (e.index == 1) {
                $.post("${oa}/weixin/auditback",{eosid:eosid,flowid:flowid,content:$("#content").val()},function(data){
                    data=JSON.parse(data);
                    if(data.success){
                        mui.toast('驳回完成！');
                        setTimeout(function () {
