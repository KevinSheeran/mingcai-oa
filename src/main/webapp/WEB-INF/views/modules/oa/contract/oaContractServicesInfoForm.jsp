...
<body>
	<form:form id="inputForm" modelAttribute="oaContractServicesInfo" action="${ctx}/oa/contract/oaContractServicesInfo/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
		<form:hidden path="finProductId" htmlEscape="false" maxlength="64" class="input-xlarge "/>
	<fieldset>
		<legend><div style="padding: 0px 10px; text-align: left "><a href="${ctx}/oa/contract/oaContractInfo/mainForm?finProductId=${oaFinanceProduct.id}">${oaFinanceProduct.name}</a>>>劳务合同</div> </legend>
		<table class="table-form">
			<tr>
				<td class="tit">合同别名</td>
				<td><form:input path="name" htmlEscape="false" maxlength="100" class="input-xlarge "/></td>
			</td>
				<td class="tit">合同人员：</td><td>
				<form:input path="contacts" htmlEscape="false" maxlength="100" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</td>
			</tr>
			<tr>
				<td class="tit">验收时间节点：</td><td><input name="timeNode" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
													   value="<fmt:formatDate value="${oaContractServicesInfo.timeNode}" pattern="yyyy-MM-dd HH:mm:ss"/>"
			...
			<tr>
				<td colspan="2"></td>
				<td class="tit">纳税识别号：</td><td><form:input path="dutyParagraph"  htmlEscape="false" maxlength="100" class="input-xlarge "/></td>
			</tr>
			<tr>
				<td colspan="2"></td>
			...
			<tr><td colspan="4">
				<div class="form-actions">
					<shiro:hasPermission name="oa:contract:oaContractInfo:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保存合同信息"/>&nbsp;</shiro:hasPermission>
				</div>
			</td></tr>
			<tr><td colspan="4">
				<h4 style="padding: 10px 0;padding-left:10px; color: #666;">
					...
					<c:if test="${oaContractServicesInfo.id!=null&&oaContractServicesInfo.id!=''}">&nbsp;&nbsp;<a class="btn " style="" onclick="addPayment('${oaContractServicesInfo.id}')"><i class="icon-plus"></i>&nbsp;添加付款信息</a></c:if>
					</shiro:hasPermission>
				</h4>
			</td></tr>
			<tr>
				<td colspan="4" style="padding-left:15px;">
					<c:if test="${oaContractServicesInfo.id!=null&&oaContractServicesInfo.id!=''}">
			...
			<tr><td colspan="4">
				<h4 style="padding: 10px 0; padding-left:10px; color: #666;">
					开票信息<shiro:hasPermission name="oa:contract:oaContractInvoice:edit">&nbsp;&nbsp;<c:if test="${oaContractServicesInfo.id!=null&&oaContractServicesInfo.id!=''}"><a class="btn " style="" onclick="addnvoice('${oaContractServicesInfo.id}')"><i class="icon-plus"></i>&nbsp;添加开票</a></c:if></shiro:hasPermission>
				</h4>
			</tr>
			<tr>
				<td colspan="4" style="padding-left:15px;">
					<c:if test="${oaContractServicesInfo.id!=null&&oaContractServicesInfo.id!=''}">
						<iframe src="${ctx}/oa/contract/oaContractInvoice?contractId=${oaContractServicesInfo.id}" name="invoice" id="invoice" style="overflow:visible;" scrolling="yes" frameborder="no" width="100%" height="250">
			...
			<tr><td colspan="4">
				<h4 style="padding: 10px 0; padding-left:10px; color: #666;">
					...
					<c:if test="${oaContractServicesInfo.id!=null&&oaContractServicesInfo.id!=''}">&nbsp;&nbsp;<a class="btn " style="" onclick="addTrem('${oaContractServicesInfo.id}')"><i class="icon-plus"></i>&nbsp;添加项</a></c:if> </shiro:hasPermission>&nbsp;&nbsp;<span id="totalMoney"></span>
			...
			<tr>
				<td colspan="4" style="padding-left:15px;">
					<c:if test="${oaContractServicesInfo.id!=null&&oaContractServicesInfo.id!=''}">
						<iframe src="${ctx}/oa/contract/oaContractPurchaseTerms/getTremlist?purchaseId=${oaContractServicesInfo.id}" name="trem" id="trem" style="overflow:visible;" scrolling="yes" frameborder="no" width="100%" height="250">
	...
	<script type="application/javascript">
        ...
        function saveTrem(id,obj){
            ...
            loading("保存中!");
            $.post("${ctx}/oa/contract/oaContractPurchaseTerms/edit",{"id":id,"number":number,"unitPrice":unitPrice},function(data){
                if(data!=null&&data!=""&&data!="null"&&data!=undefined){
                    loading("保存完成!");
                    closeLoading();
                    document.getElementById('trem').contentWindow.$("#total_"+id).html(parseFloat(number)*parseFloat(unitPrice).toFixed(2));
                    document.getElementById('trem').contentWindow.$("#divnumber_"+id).html(parseInt($(obj).attr("maxnumber"))-number);
	...
	<!--付款条约-->
        ...
        function addPayment(contractId,id){
            ...
            $.jBox.open("iframe:${ctx}/oa/contract/oaContractPayment/form?contractId="+contractId+"&id="+id+"&type=3", title, width,height, {
                buttons:{"保存":"ok", "关闭":true}, submit:function(v, h, f){
                    if (v=="ok"){
                        var formdata = h.find("iframe")[0].contentWindow.$("#inputForm");
                        if(formdata!='undefined'){
...
<!--开票管理-->
        ...
        function addnvoice(contractId,id){
            ...
            $.jBox.open("iframe:${ctx}/oa/contract/oaContractInvoice/form?contractId="+contractId+"&types=3&id="+id, title, width,height, {
                buttons:{"保存":"ok", "关闭":true}, submit:function(v, h, f){
                    if (v=="ok"){
                        var formdata = h.find("iframe")[0].contentWindow.$("#inputForm");
                        if(formdata!='undefined'){
