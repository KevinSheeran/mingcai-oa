...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/oa/user/oaUserAccountLog/">月结记录列表</a></li>
		<li class="active"><a href="${ctx}/oa/user/oaUserAccountLog/form?id=${oaUserAccountLog.id}">月结记录<shiro:hasPermission name="oa:user:oaUserAccountLog:edit">${not empty oaUserAccountLog.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="oa:user:oaUserAccountLog:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="oaUserAccountLog" action="${ctx}/oa/user/oaUserAccountLog/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>		
		<div class="control-group">
			<label class="control-label">当前申请销售额id：</label>
			<div class="controls">
				<form:input path="salesVolumeId.id" htmlEscape="false" maxlength="64" class="input-xlarge required"/>
		...
		<div class="form-actions">
			<shiro:hasPermission name="oa:user:oaUserAccountLog:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
