...
<img id="${id}Icon" width="90" src="${not empty value?value:''}"/>&nbsp;<label id="${id}IconLabel">${not empty value?value:'无'}</label>&nbsp;
<input id="${id}" name="${name}" type="hidden" value="${value}"/><a id="${id}Button" href="javascript:" class="btn">选择</a>&nbsp;&nbsp;
<script type="text/javascript">
	$("#${id}Button").click(function(){
		$.jBox.open("iframe:${ctx}/tag/iconselect?value="+encodeURI($("#${id}").val()), "选择图标", 700, 500, {
