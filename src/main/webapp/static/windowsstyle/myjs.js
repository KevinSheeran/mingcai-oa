...
myLib.desktop.win={
    winHtml:function(title,url,id){
        return "<div class='windows corner rgba' id="
            ...
            +title
            +"</b><span class='win_btnblock'><a href='#' class='winMinimize' title='最小化'></a><a href='#' class='winMaximize' title='最大化'></a><a href='#' class='winHyimize' title='还原'></a><a href='#' class='winClose' title='关闭'></a></span></div></div>";
    },
    winiframe:function(url,id){
        return "<iframe frameborder='0' name='myFrame_"
            +id
            +"' id='myFrame_"
            +id
            +"' class='winframe' style='display: none;' scrolling='auto' width='100%' src='"
            +url
            +"'></iframe>";
    },
    ...
    closeWin:function(obj){
        ...
        obj.animate({
            ...
            opacity:0,
            top: $(window).height()/2,
            left:  $(window).width()/2},500,function() {
                $(this).remove();
            }
        );
    ...
    maximizeWin:function(obj){
        var myData=myLib.desktop.getMydata(),
            wh=myData.winWh;//获取当前document宽高
        obj
            .css({'width':wh['w']-5,'height':wh['h']-45,'left':0,'top':0})
            .draggable( "disable" )
            .resizable( "disable" )
            .fadeTo("fast",1)
            .find(".winframe")
            .css({'width':wh['w']-25,'height':wh['h']-96});
    },
    //还原窗口函数
    hyimizeWin:function(obj){
        ...
        obj.css({'width':winLocation['w'],'height':winLocation['h'],'left':winLocation['left'],'top':winLocation['top']})
            ...
            .resizable( "enable" )
            .find(".winframe")
            .css({'width':winLocation['w']-9,'height':winLocation['h']-45});
    },
    //交换窗口z-index值
    ...
    newWin:function(options){
        ...
        var defaults = {
            ...
            WindowWidth:          Math.round(wh['w']*0.9),           /* Only pixels */
        ...
        if($("#"+id).size()<=0){
            ...
            if(options.WindowAnimation=='none'){
                ...
                $newWin
                    .css({"left":$(window).width()/2,"top":$(window).height()/2,"z-index":zindex,"opacity":0})
                    .addClass("loading")
                    .show()
                    .animate({
                        ...
                        opacity:1,
                        top: wTop,
                        left: wLeft}, 500,function(){
                        $newWin.append($(_this.winiframe(options['iframSrc'],id)));
                        $("#myFrame_"+id).load(function(){                             //  等iframe加载完毕
                            $(this).show();
                        });
                        $newWin
                            .data('winLocation',{
                            ...
                            .find(".winframe")
                            .css({"width":options['WindowWidth']-9,"height":options['WindowHeight']-45})
                        //等待iframe加载完毕
                        .load(function(){
                        //调用窗口拖动,参数可拖动的范围上下左右，窗口id和，浏览器可视窗口大小
                        if(options['WindowDraggable']){
                            _this.drag([0,0,wh['w']-options['WindowWidth']-10,wh['h']-options['WindowHeight']-50],id,wh);
                        }
                        //调用窗口resize,传递最大最小宽度和高度，新窗口对象id，浏览器可视窗口大小
                        if(options['WindowResizable']){
                            _this.resize(options['WindowMinWidth'],options['WindowMinHeight'],wh['w']-wLeft,wh['h']-wTop-50,id,wh);
                        }
    ...
    upWinResize_block:function(win){
        ...
        win.resizable( "option" ,{'maxWidth':$(window).width()-offset.left-10,'maxHeight':$(window).height()-offset.top-45})
    },
    upWinDrag_block:function(win){
        ...
        //更新窗口可拖动区域大小
        win.draggable( "option", "containment", [10,10,$(window).width()-w-10,$(window).height()-h-45] )
    },
    ...
    resize:function(minW,minH,maxW,maxH,win_id,wh){
        ...
        $("#"+win_id)
            ...
            .bind( "resize", function(event, ui) {
                ...
                $(this).children(".winframe").css({"width":w-9,"height":h-45});
...
myLib.desktop.lrBar={
    init:function(){
        ...
        var $default_tools=myData.panel.lrBar['default_tools']
            ...
            ,$user_item=myData.panel.lrBar['user_item']
            ,wh=myData.winWh;
        ...
        $start_item
            ...
            var title=$(this).attr('title'),wid=$(this).attr('id');
            var href= $(this).attr('path');
            myLib.desktop.win.newWin({
                WindowTitle:title,
                iframSrc:href,
        ...
        $start_btn.click(function(event) {
            ...
            {
            $start_item.hide();
                $start_item.removeClass("active");
                $("#start_block").removeClass("active");
        }
        });
...
myLib.desktop.deskIcon={
    ...
    arrangeIcons:function(){
        ...
        //设置桌面图标容器元素区域大小
        $deskIconBlock.css({"width":(winWh['w'])+"px","height":(winWh['h'])+"px","margin-top":"0px",'margin-left':'0px'});
        //对图标定位
        ...
        var rows=Math.floor((winWh['h']-0)/gH);
    ...
    init:function(){
        ...
        var data=[
            ...
            ,[{
                ...
                func:function(){
                    myLib.desktop.win.newWin({
                        WindowTitle:'主题设置&nbsp;&nbsp;&nbsp;Web 应用背景图',
                        iframSrc:'/a/sys/user/bglist',
                        WindowsId:'bg_12345',
                        WindowAnimation:'easeInBack'
                    });
                }
            ...
            ,[{
                text:"退出系统",
                func:function(){$(".loginout").first().click();}
            }]
        ];
