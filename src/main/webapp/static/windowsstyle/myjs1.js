...
myLib.desktop.win={
    ...
    maximizeWin:function(obj){
        var myData=myLib.desktop.getMydata(),
            wh=myData.winWh;//获取当前document宽高
        obj
            .css({'width':wh['w'],'height':wh['h']-45,'left':0,'top':0})
            .draggable( "disable" )
            .resizable( "disable" )
            .fadeTo("fast",1)
            .find(".winframe")
            .css({'width':wh['w']-9,'height':wh['h']-86});
    },
    //还原窗口函数
    hyimizeWin:function(obj){
        ...
        obj.css({'width':winLocation['w'],'height':winLocation['h'],'left':winLocation['left'],'top':winLocation['top']})
            ...
            .resizable( "enable" )
            .find(".winframe")
            .css({'width':winLocation['w']-9,'height':winLocation['h']-40});
    },
    //交换窗口z-index值
    ...
    newWin:function(options){
        ...
        if($("#"+id).size()<=0){
            ...
            $newWin
                ...
                .find(".winframe")
                .css({"width":options['WindowWidth']-9,"height":options['WindowHeight']-40})
            ...
            if(options['WindowDraggable']){
                _this.drag([0,0,wh['w']-options['WindowWidth']-10,wh['h']-options['WindowHeight']-45],id,wh);
            }
            //调用窗口resize,传递最大最小宽度和高度，新窗口对象id，浏览器可视窗口大小
            if(options['WindowResizable']){
                _this.resize(options['WindowMinWidth'],options['WindowMinHeight'],wh['w']-wLeft,wh['h']-wTop-45,id,wh);
            }
    ...
    resize:function(minW,minH,maxW,maxH,win_id,wh){
        ...
        $("#"+win_id)
            ...
            .bind( "resize", function(event, ui) {
                ...
                $(this).children(".winframe").css({"width":w-9,"height":h-40});
